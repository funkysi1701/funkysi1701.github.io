...
test.describe('Homepage and Navigation', () => {
  test('Responsive design on tablet viewport', async ({ page }) => {
    await test.step('Set viewport to tablet size (768x1024)', async () => {
    ...
    await test.step('Verify layout adapts appropriately', async () => {
      // 3. Verify layout adapts appropriately
      const body = page.locator('body');
      await expect(body).toBeVisible();
    });

    await test.step('Check navigation menu display', async () => {
    ...
    await test.step('Test blog post grid/list layout', async () => {
    ...
    await test.step('Verify images scale properly', async () => {
