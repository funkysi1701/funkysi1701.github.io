...
test.describe('Accessibility', () => {
  test('Screen reader compatibility', async ({ page }) => {
    ...
    await test.step('Verify page has proper heading hierarchy (h1, h2, h3)', async () => {
    ...
    await test.step('Check that all images have alt attributes', async () => {
    ...
    await test.step('Verify links have descriptive text', async () => {
      // 4. Verify links have descriptive text
      const links = await page.locator('a').all();
      for (let i = 0; i < Math.min(links.length, 10); i++) {
        const linkText = await links[i].textContent();
        ariaLabel = await links[i].getAttribute('aria-label');
    ...
    await test.step('Check for proper ARIA labels on interactive elements', async () => {
    ...
    await test.step('Verify form fields have associated labels', async () => {
    ...
    await test.step('Check that page language is declared (lang attribute)', async () => {
